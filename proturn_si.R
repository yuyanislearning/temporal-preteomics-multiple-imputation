...
if (length(args) != 5) {
  stop("Usage: Rscript proturn_SI.R <dir> <hl.out> <imputed-data> <ids> <n-masked>", call. = FALSE)
}

# Define the directory and file locations
dir <- args[1]
hl.out_location <-  glue("{dir}/{args[2]}")
hl.data.out_location <- glue("{dir}/{args[3]}")
ids <- glue("{dir}/{args[4]}")
n_masked <- as.numeric(args[5])

# Read in IDs from the specified CSV file
id_df <- read.csv(ids, header = FALSE, stringsAsFactors = FALSE, skip = 1)

#for args[3], split by _ and get the first element
split_args <- strsplit(args[3], "_")
method <- split_args[[1]][1]
output_file <- glue("{dir}/ProTurn_{method}_{n_masked}.txt")
