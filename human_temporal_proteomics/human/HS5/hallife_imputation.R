...
tmp_dat<-read_csv('HS7_ding.csv', col_names =T)

tmp_dat=tmp_dat %>% filter(!is.na(k))

sum(!is.na(match(tmp_dat$Uniprot, (unique(dat$Uniprot)) )))
#205 matches out of 282 in dings file
...
data_filt<-data[cnt_na < 8,]  # 14 FOR 15 tp...9 for 10 tps..8 FOR 9 TPS
#data_filt<-data[cnt_na < 5,]  # male is one less 
data_filt1<-subset(data_filt,select=-c(ID))
#data_filt1[data_filt1 > 1]<-1

mis_per<-sum(is.na(data_filt1))/prod(dim(data_filt1)) #39% missing aj ctrl
...
mice_impute <- mice(data_filt1,m=10,method=c('pmm','pmm','pmm','pmm','pmm',
                                             'pmm','pmm','pmm','pmm'), maxiter=10)     #female
#mice_impute <- mice(data_filt1,m=10,method=c('pmm','pmm','pmm','pmm','pmm',
#                                            'pmm'), maxiter=10)     #male
...
for(i in 1:10) {
  ...
  colnames(mice_imp) <- c("ID","1","2","4","5","8","9","10","12","14")
