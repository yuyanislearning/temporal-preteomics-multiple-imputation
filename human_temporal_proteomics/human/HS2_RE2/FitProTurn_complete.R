...
hl.data.out_location <- "hs2_imp.1.out"   ############################3

output_file <- "ProTurn_Output_hs2_imp1.txt"  ######################################
...
if (Refit == TRUE){
  for (c in 1:nrow(dt)) {
    ...
    se_new<-(sum((ds$A0-Refitted_Predicted)^2)/14)^0.5  #6 for seven tps....5 for 6 tps  (female n male)
    print(se_new)
...
new_oupt <- read.table("ProTurn_Output_hs2_imp1.txt", header=TRUE, fill=TRUE, skip = 0)  #################3
...
imp1 <- read.table("ProTurn_Output_hs2_imp1.txt", header=TRUE, fill=TRUE, skip = 0)
...
colnames(median_hl) <- c("UniProt","median.K")   #330 with OR filter
...
imp2 <- read.table("ProTurn_Output_hs2_imp2.txt", header=TRUE, fill=TRUE, skip = 0)
dat_imp<-imp2
newdata_imp <- subset(dat_imp, R2 >= 0.8 | SS<0.05 )
#newdata_imp <- subset(dat_imp)
...
imp3 <- read.table("ProTurn_Output_hs2_imp3.txt", header=TRUE, fill=TRUE, skip = 0)
dat_imp<-imp3
newdata_imp <- subset(dat_imp, R2 >= 0.8 | SS<0.05   )
#newdata_imp <- subset(dat_imp)
...
imp4 <- read.table("ProTurn_Output_hs2_imp4.txt", header=TRUE, fill=TRUE, skip = 0)
dat_imp<-imp4
newdata_imp <- subset(dat_imp, R2 >= 0.8 | SS<0.05 )
#newdata_imp <- subset(dat_imp)
...
imp5 <- read.table("ProTurn_Output_hs2_imp5.txt", header=TRUE, fill=TRUE, skip = 0)
dat_imp<-imp5
newdata_imp <- subset(dat_imp, R2 >= 0.8 | SS<0.05 )
#newdata_imp <- subset(dat_imp)
...
imp6 <- read.table("ProTurn_Output_hs2_imp6.txt", header=TRUE, fill=TRUE, skip = 0)
dat_imp<-imp6
newdata_imp <- subset(dat_imp, R2 >= 0.8 | SS<0.05 )
#newdata_imp <- subset(dat_imp)
...
imp7 <- read.table("ProTurn_Output_hs2_imp7.txt", header=TRUE, fill=TRUE, skip = 0)
dat_imp<-imp7
newdata_imp <- subset(dat_imp, R2 >= 0.8 | SS<0.05 )
#newdata_imp <- subset(dat_imp)
...
imp8 <- read.table("ProTurn_Output_hs2_imp8.txt", header=TRUE, fill=TRUE, skip = 0)
dat_imp<-imp8
newdata_imp <- subset(dat_imp, R2 >= 0.8 | SS<0.05 )
#newdata_imp <- subset(dat_imp)
...
imp9 <- read.table("ProTurn_Output_hs2_imp9.txt", header=TRUE, fill=TRUE, skip = 0)
dat_imp<-imp9
newdata_imp <- subset(dat_imp, R2 >= 0.8| SS<0.05  )
#newdata_imp <- subset(dat_imp)
...
imp10 <- read.table("ProTurn_Output_hs2_imp10.txt", header=TRUE, fill=TRUE, skip = 0)
dat_imp<-imp10
newdata_imp <- subset(dat_imp, R2 >= 0.8 | SS<0.05 )
#newdata_imp <- subset(dat_imp)
...
write.csv(median_hl_impT,"hs2_kwithImp_filtered.csv")
