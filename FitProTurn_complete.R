...
hl.data.out_location <- "aj_ctrl_imp.6.out"   ############################3

output_file <- "ProTurn_Output_aj_ctrl_imp6.txt"  ######################################
...
if (Refit == TRUE){
  for (c in 1:nrow(dt)) {
    ...
    se_new<-(sum((ds$A0-Refitted_Predicted)^2)/6)^0.5  #6 for seven tps....5 for 6 tps  (female n male)
    print(se_new)
...
new_oupt <- read.table("ProTurn_Output_aj_ctrl_imp1.txt", header=TRUE, fill=TRUE, skip = 0)  #################3
...
imp1 <- read.table("ProTurn_Output_aj_ctrl_imp1.txt", header=TRUE, fill=TRUE, skip = 0)
imp2 <- read.table("ProTurn_Output_aj_ctrl_imp2.txt", header=TRUE, fill=TRUE, skip = 0)
imp3 <- read.table("ProTurn_Output_aj_ctrl_imp3.txt", header=TRUE, fill=TRUE, skip = 0)
imp4 <- read.table("ProTurn_Output_aj_ctrl_imp4.txt", header=TRUE, fill=TRUE, skip = 0)
imp5 <- read.table("ProTurn_Output_aj_ctrl_imp5.txt", header=TRUE, fill=TRUE, skip = 0)
imp6 <- read.table("ProTurn_Output_aj_ctrl_imp6.txt", header=TRUE, fill=TRUE, skip = 0)
imp7 <- read.table("ProTurn_Output_aj_ctrl_imp7.txt", header=TRUE, fill=TRUE, skip = 0)
imp8 <- read.table("ProTurn_Output_aj_ctrl_imp8.txt", header=TRUE, fill=TRUE, skip = 0)
imp9 <- read.table("ProTurn_Output_aj_ctrl_imp9.txt", header=TRUE, fill=TRUE, skip = 0)
imp10 <- read.table("ProTurn_Output_aj_ctrl_imp10.txt", header=TRUE, fill=TRUE, skip = 0)
...
#newdata_noimp <- subset(dat_noimp,DP >= 4 )
newdata_noimp <- subset(dat_noimp,DP >= 4 & R2 >= 0.8 )
hl_order<-dat_noimp[order(dat_noimp$Uniprot),]
#newdata_noimp <- subset(newdata_noimp, R2 >= 0.8)
#newdata_noimp <- subset(newdata_noimp, SS <=0.05)
median_hl <- aggregate(newdata_noimp$k,by = list(newdata_noimp$Uniprot),median) #2724 PROTEINS >4 tps, without R2 and SE, 1717 after R2 filter
...
median_hl_impT <- aggregate(impT$median.K, by = list(impT$UniProt),median) # 3568 without R2, 2620 after R2
colnames(median_hl_impT) <- c("UniProt","median.K")
write.csv(median_hl_impT,"aj_ctrl_kwithImp_filtered.csv")


median_hl_impT_nolarge<-subset(median_hl_impT , x <= 10 & x > 0  ) # 2593 for aj ctrl
colnames(median_hl_impT_nolarge) <- c("UniProt","median.K")

###All peptides from 10 imputaions stacked 
impT <- rbind(imp1,imp2,imp3,imp4,imp5,imp6,imp7,imp8,imp9,imp10) 
newdata_imp <- subset(impT, R2 >= 0.8  )
write.csv(newdata_imp,"aj_ctrl_ImpFiltured_PeptidesStacked.csv")


library(data.table)
dat_sage <- fread("hl_sage.out")
dat_sage <- subset(dat_sage, R2 >= 0.8  )
median_hl_sage <- aggregate(dat_sage$k, by = list(dat_sage$Uniprot),median) # 3568 without R2, 2617 after R2
colnames(median_hl_sage) <- c("UniProt","median.K")


hl_order<-dat_sage[order(dat_sage$Uniprot),]
